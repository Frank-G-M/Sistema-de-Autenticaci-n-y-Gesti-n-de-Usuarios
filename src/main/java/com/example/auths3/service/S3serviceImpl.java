...
public class S3serviceImpl implements IS3Service{
    ...
    public String createBucket(String bucketName) {
        ...
        return "Bucket creado en "+ response.location();
    }

    @Override
    public String validatedBucket(String bucketName) {
        try{
            this.s3Client.headBucket(headBucket -> headBucket.bucket(bucketName));
            return "Bucket si existe: " + bucketName;
        } catch (S3Exception exception) {
            return "Bucket no existe: " + bucketName;
        }
    }
    ...
    public List<String> getAllBucket() {
        ...
        if (bucketsResponse.hasBuckets()) {
            return bucketsResponse.buckets()
                    .stream()
                    .map(Bucket::name)//bucket -> bucket.name() es mas resumido
                    .toList();
        }else {
            return List.of();
        }
    ...
    public void downloadFile(String bucket, String key) throws IOException {
        ...
        try(FileOutputStream  fos = new FileOutputStream(file)){
            ...
            throw new IOException("Error en descarga en: "+ exception.getCause());
        }
    }

    @Override
    public String generetePresignedUploadUrl(String bucketName, String key, Duration duration) {
        PutObjectRequest putObjectRequest = PutObjectRequest.builder()
                .bucket(bucketName)
                .key(key)
                .build();
    ...
    @Override
    public String generetePresignedDownloadUrl(String bucketName, String key, Duration duration) {
        GetObjectRequest getObjectRequest = GetObjectRequest.builder()
                .bucket(bucketName)
                .key(key)
                .build();
